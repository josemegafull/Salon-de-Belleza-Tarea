...
//jPanel3.setLayout(new GridLayout(2,0)); /*jPanel4.setLayout(new GridLayout(0,2));*/
    ...
    private void initComponents() {
        ...
        jPanel4.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "SERVICIOS", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.TOP, new java.awt.Font("Tahoma", 1, 36), new java.awt.Color(38, 150, 209))); // NOI18N
    ...
    private void jLabel8MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel8MouseClicked
        if (seleccion<0){ VC.MSimple(" Debe elegir un registro a editar","Seleccion",JOptionPane.INFORMATION_MESSAGE);  return;}//----- Cierre      
             ...
             jLabel11.setVisible(true); jTextField2.requestFocus(); 
    ...
    private void jLabel12MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel12MouseClicked
        ...
        if(valor==JOptionPane.YES_OPTION){
         ...
         //jScrollPane1.setVisible(true); jTable1.setVisible(true);  jTable1.setModel(OB.MostrartablaServicios(jTable1));
    ...
    private void jLabel10MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel10MouseClicked
        String para;ModeloBD MD = new ModeloBD();
         if (seleccion<0){VC.MSimple(" Debe seleccionar un registro a eliminar"," Error !!!",JOptionPane.ERROR_MESSAGE);return;}
            ...
            ConfiguracionDInicio();
           //jTable1.setModel(OB.MostrartablaServicios(jTable1));    VC.tamañocolumnas(jTable1, "empleados");
           seleccion=-1;    pos=-1;     
    ...
    for (int i = 0; i < Objetos.SERV.size(); i++) {
        Servicio emp = Objetos.SERV.get(i);        
        if(String.valueOf(jTable1.getValueAt(pos,0)).equals(emp.getCodigo())){
            cod=String.valueOf(jTable1.getValueAt(pos,0));
            vl=i;
        }//------->Cierre
        }
    return vl;
    ...
    private void filtro(){
        tr=new TableRowSorter<>(OB.MostrartablaServicios(jTable1));
...
capturardatos();    ModeloBD MD = new ModeloBD();   String vl = "";
    try{
        switch(valid){
            case"nuevo":
                OB.AGSerivio(new Servicio(registro[0],registro[1],registro[2],Double.parseDouble(registro[3]),Double.parseDouble(registro[4])));
                if (MD.INServicio(registro[0]).length()>0){ VC.MSimple("No se pudo guardar el registro \n Error: "+MD.INServicio(registro[0]),"Error al guardar los datos",JOptionPane.ERROR_MESSAGE);return;}//----->Cierre
                vl=" Registro guardado de forma exitosa";  // jTable1.setVisible(true);   jScrollPane1.setVisible(true);    
            break;
            case"editar":
                OB.EDSerivio(new Servicio(registro[0],registro[1],registro[2],Double.parseDouble(registro[3]),Double.parseDouble(registro[4])),Buscarposicion());
              
                if (MD.EDServicio(registro[0]).length()>0){VC.MSimple("No se pudo actualizar el registro \n Error: "+MD.EDServicio(registro[0]),"Error al guardar los datos",JOptionPane.ERROR_MESSAGE);return;}//---->Cierre
    ...
    private void cargardatosentext (){
        for (int i = 0; i < Objetos.SERV.size(); i++) {
            Servicio em= Objetos.SERV.get(i);
           if(String.valueOf(jTable1.getValueAt(pos,0)).equals(em.getCodigo())){
           jTextField5.setText(em.getCodigo());  jTextField1.setText(em.getNombre());    jTextField2.setText(em.getDescripcion());
           jTextField3.setText(em.getPrecio()+"");  jTextField4.setText(em.getDuracion()+"");
        } 
    }  
  }        
 /* --------------------------------------------------------------------------------------------------------- 
...
private String cedula(){
 String resp="";
    for (int i = 0; i < Objetos.SERV.size(); i++) {
        ...
        switch (valid){
         case "editar":
            if(emp.getCodigo().equalsIgnoreCase(jTextField1.getText())){
                resp="Ya existe un registro con el codigo ingresado, la codigo es un campo unico!!!"; return resp;}//------>Cierre 
           break;
         case "nuevo":
            if(emp.getCodigo().equalsIgnoreCase(jTextField1.getText())){resp="Ya existe un registro con la cedula ingresada";return resp;}//---Cierre 
            break;      
        }
    }
 return resp;
...
private void ConfiguracionDInicio(){
    ...
    valid="";
    jPanel10.setVisible(true);
    jTable1.setModel(OB.MostrartablaServicios(jTable1));
    VC.manejolabelsbotones(jPanel2,"ocultar");   VC.tamañocolumnas(jTable1, "empleados");     VC.manejocajas(jPanel5,"noeditar");             
    VC.manejocajas(jPanel5, "limpiar");                     VC.mamejocombos(jPanel5,"bloquear");               
