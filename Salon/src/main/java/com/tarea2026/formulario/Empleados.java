...
//jPanel3.setLayout(new GridLayout(2,0)); /*jPanel4.setLayout(new GridLayout(0,2));*/
    ...
    private void initComponents() {
        ...
        jPanel4.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "PROFESIONAL", javax.swing.border.TitledBorder.CENTER, javax.swing.border.TitledBorder.TOP, new java.awt.Font("Tahoma", 1, 36), new java.awt.Color(38, 150, 209))); // NOI18N
    ...
    private void jLabel12MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel12MouseClicked
        ...
        if(valor==JOptionPane.YES_OPTION){
         ...
         jScrollPane1.setVisible(true); jTable1.setVisible(true);  jTable1.setModel(OB.MostrartablaEmpleados(jTable1));
                ...
                if(valor==JOptionPane.YES_OPTION){
                    para=jTextField1.getText();
                     if (MD.eliminaremp(para).equals("completada")){
                        ...
                        jTable1.setModel(OB.MostrartablaEmpleados(jTable1));
                        VC.tamañocolumnas(jTable1, "empleados");
                        seleccion=-1;   pos=-1; 
                    }  
                 return;
              } 
           jTable1.setModel(OB.MostrartablaEmpleados(jTable1));    VC.tamañocolumnas(jTable1, "empleados");
           seleccion=-1;    pos=-1;     
    ...
    for (int i = 0; i < Objetos.PRO.size(); i++) {
        Profesional emp = Objetos.PRO.get(i);        
    ...
    private void filtro(){
        tr=new TableRowSorter<>(OB.MostrartablaEmpleados(jTable1));
...
capturardatos();    ModeloBD MD = new ModeloBD();   String vl = "";
    try{
        switch(valid){
            case"nuevo":
                OB.AGEmpleados(new Profesional(registro[0],registro[1],registro[2],registro[3],registro[4]));
                if (MD.INempleado(registro[0]).length()>0){ VC.MSimple("No se pudo guardar el registro \n Error: "+MD.INempleado(registro[1]),"Error al guardar los datos",JOptionPane.ERROR_MESSAGE);return;}//----->Cierre
                vl=" Registro guardado de forma exitosa";  // jTable1.setVisible(true);   jScrollPane1.setVisible(true);    
            break;
            case"editar":
                OB.EDEmpleados(new Profesional(registro[0],registro[1],registro[2],registro[3],registro[4]),Buscarposicion());
                if (MD.EDempleado(registro[0]).length()>0){VC.MSimple("No se pudo actualizar el registro \n Error: "+MD.INempleado(registro[1]),"Error al guardar los datos",JOptionPane.ERROR_MESSAGE);return;}//---->Cierre
    ...
    private void capturardatos (){//recoger datos de los text
        ...
        registro[0]= jTextField1.getText(); 
        registro[1]= jTextField2.getText();
        registro[2]= jTextField3.getText();
        registro[3]= jTextField4.getText();
        registro[4]= jTextField5.getText();
      }

    private void cargardatosentext (){
        for (int i = 0; i < Objetos.PRO.size(); i++) {
           Profesional em= Objetos.PRO.get(i);
           ...
           jTextField4.setText(em.getCorreo());  jTextField5.setText(em.getEspecialidad());
        } 
    }  
  }        
 /* --------------------------------------------------------------------------------------------------------- 
...
private void ActivarNuevo(){
    ...
    VC.mamejocombos(jPanel5,"desbloquear"); jTextField2.requestFocus();
...
private void ConfiguracionDInicio(){
    ...
    valid=""; jTable1.setVisible(true);    jTable1.setModel(OB.MostrartablaEmpleados(jTable1));   VC.tamañocolumnas(jTable1,"empleados");
